...
```sh
scp -r deploy ubuntu@92.5.105.170:/tmp/
ssh -t ubuntu@92.5.105.170 'cd /tmp && ./deploy/bootstrap-vps.sh --url http://92.5.105.170:8080'
```
...
```sh
ssh -t ubuntu@92.5.105.170 'cd /opt/fixmate && ./deploy/deploy.sh ghcr.io/beahsan/fixmate/app:manual'
...
```sh
ssh -t ubuntu@92.5.105.170 'cd /opt/fixmate && ./deploy/rollback.sh'
```
